...
export const ReviewsRatingTable =  pgTable("reviews_rating", {
    ...
    review_date: timestamp("review_date").default(sql`NOW()`).default(new Date()),
...
export const PromotionalOffersTable = pgTable("promotional_offers", {
    ...
    description: text("description").notNull(),
    start_date: timestamp("start_date").default(sql`NOW()`).default(new Date()),
    end_date: timestamp("end_date").default(sql`NOW()`).default(new Date()),
